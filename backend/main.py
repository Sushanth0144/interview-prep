...
def create_app():
    ...
    origins_list = [o.strip() for o in raw.split(",") if o.strip()]

    expanded = []
    for o in origins_list:
        if o == "https://*.vercel.app":
            expanded.append(re.compile(r"^https://.*\.vercel\.app$"))
        else:
            expanded.append(o)
    ...
    with app.app_context():
        ...
        # ---- One-time seed on first boot (safe check) ----
        try:
            if not User.query.filter_by(username="SushanthK1").first():
                admin_user = User(
                    username="SushanthK1",
                ...
                if not Topic.query.filter_by(name="HTML").first():
                    html = Topic(
                        name="HTML",
                ...
                db.session.commit()
                app.logger.info("✅ Seeded admin/user (and sample topic).")
            else:
                app.logger.info("ℹ️ Seed skipped: users already exist.")
        except Exception as e:
            app.logger.warning(f"Seed step skipped/failed: {e}")
